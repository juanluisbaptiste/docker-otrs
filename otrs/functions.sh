...
function upgrade () {
  print_warning "OTRS \e[92mMAJOR VERSION UPGRADE\e[0m, press ctrl-C if you want to CANCEL !! (you have 10 seconds)"
  sleep 10
  ...
  if [ $? -eq 0  ]; then
    cat ${OTRS_ROOT}/scripts/DBUpdate-to-5.mysql.sql | ${mysqlcmd} ${OTRS_DATABASE}  &> ${upgrade_log}
    if [ $? -gt 0  ]; then
      print_error "Cannot upgrade database" | tee ${upgrade_log} && exit 1
    fi
    su -c "${OTRS_ROOT}/bin/otrs.Console.pl Maint::Database::Check" -s /bin/bash otrs  &> ${upgrade_log}
    if [ $? -gt 0  ]; then
      print_error "Database upgrade check failed" | tee ${upgrade_log} && exit 1
    fi
    su -c "/opt/otrs//scripts/DBUpdate-to-5.pl" -s /bin/bash otrs  &> ${upgrade_log}
    if [ $? -gt 0  ]; then
  ...
  if [ "${installed_modules}" == "" ]; then
    ...
    for i in ${installed_modules}; do
      print_info "[+] Upgrading module \e[92m${i}\e[0m..." | tee ${upgrade_log}
      ...
      if [ "${latest_version}" != "" ]; then
        print_info "- Upgrading to version \e[92m${latest_version}\e[0m..."  | tee ${upgrade_log}
